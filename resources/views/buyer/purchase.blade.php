      ...
      if(content['success'] != ''){
        ...
        for(j = 0 ; j<content['order_list'].length ; j++){
              $("#order_list_"+content['order_list'][j]['order_id']).append('<div id="div_order_'+content['order_list'][j]['order_id']+'">'+
                (content['order_list'][j]['status'] == 0 ? '<button type="button" onclick="payment('+content['order_list'][j]['order_id']+','+content['order_list'][j]['id']+','+content['order_list'][j]['total_price']+')" class="btn btn-w-m btn-info text-left">OrderID: #'+content['order_list'][j]['order_id']+'&nbsp แจ้งโอนเงิน ที่นี่ จำนวนเงินที่ต้องโอน: '+content['order_list'][j]['total_price'].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+' &nbsp&nbsp&nbsp สถานะ : รอการจ่ายเงิน</button>':'')+
                (content['order_list'][j]['status'] == 1 ? '<button type="button" class="btn btn-w-m btn-warning text-left">OrderID: #'+content['order_list'][j]['order_id']+'&nbsp เมื่อผู้ขายยืนยันแล้วระบบจะอัพเดทให้อัตโนมัติ &nbsp&nbsp&nbsp สถานะ : รอตรวจสอบการชำระเงิน</button>':'')+
                (content['order_list'][j]['status'] == 2 ? '<button type="button" class="btn btn-w-m btn-warning text-left">OrderID: #'+content['order_list'][j]['order_id']+'&nbsp รอผู้ขายแจ้งหมายเลข Tracking &nbsp&nbsp&nbsp สถานะ : กำลังส่งของ</button>':'')+
                (content['order_list'][j]['status'] == 3 ? '<button type="button" class="btn btn-w-m btn-success text-left">OrderID: #'+content['order_list'][j]['order_id']+'&nbsp ได้รับสินค้าแล้วรอการรีวิว &nbsp&nbsp&nbsp สถานะ : ได้รับสินค้าแล้ว</button>':'')+
                (content['order_list'][j]['status'] == 4 ? '<button type="button" class="btn btn-w-m btn-danger text-left">OrderID: #'+content['order_list'][j]['order_id']+'&nbsp คำสั่งซื้อถูกยกเลิก &nbsp&nbsp&nbsp สถานะ : ยกเลิก</button>':'')+
                (content['order_list'][j]['status'] == 5 ? '<button type="button" class="btn btn-w-m btn-default text-left">OrderID: #'+content['order_list'][j]['order_id']+'&nbsp หมดเวลาชำระเงิน &nbsp&nbsp&nbsp สถานะ : เกินกำหนดเวลาชำระเงิน</button>':'')+
                '</div>');  
          }
